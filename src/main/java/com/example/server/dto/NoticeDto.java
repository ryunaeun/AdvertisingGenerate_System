...
public class NoticeDto {
    ...
    public static class NoticePost {
        ...
        // DTO -> 엔티티 변환 메서드
        public Notice toEntity(String email) {
            logger.debug("Converting NoticePost DTO to Entity. Email: {}", email);
            logger.debug("Title: {}, Content: {}", title, content);

            Notice notice = new Notice();
            notice.setEmail(email);        // 로그인한 사용자 이메일 설정
